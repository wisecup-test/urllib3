...
1. MUST: Socket binding operations MUST use sock.bind((host, port)) with explicit host and port parameters for all TCP server socket initialization

## Policy Block

- MUST Socket binding operations MUST use sock.bind((host, port)) with explicit host and port parameters for all TCP server socket initialization
