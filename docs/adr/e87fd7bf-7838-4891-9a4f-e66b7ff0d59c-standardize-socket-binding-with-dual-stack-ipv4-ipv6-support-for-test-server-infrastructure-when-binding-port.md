...
1. MUST: When binding to port 0 for random port allocation, implementations MUST reuse the allocated port number from the first socket for subsequent address family bindings to ensure consistent port across IPv4 and IPv6

## Policy Block

- MUST When binding to port 0 for random port allocation, implementations MUST reuse the allocated port number from the first socket for subsequent address family bindings to ensure consistent port across IPv4 and IPv6
