...
1. MAY: Platform-specific transports (Emscripten fetch, PyOpenSSL) MAY adapt send semantics to underlying APIs (js_xhr.send(), connection.send()) while maintaining byte-oriented interface contracts

## Policy Block

- MAY Platform-specific transports (Emscripten fetch, PyOpenSSL) MAY adapt send semantics to underlying APIs (js_xhr.send(), connection.send()) while maintaining byte-oriented interface contracts
