...
1. SHOULD: Transport abstraction layers (SSLTransport, HTTP2Connection, pyOpenSSL wrappers) SHOULD delegate to underlying socket send() rather than implementing custom buffering

## Policy Block

- SHOULD Transport abstraction layers (SSLTransport, HTTP2Connection, pyOpenSSL wrappers) SHOULD delegate to underlying socket send() rather than implementing custom buffering
