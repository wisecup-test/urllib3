...
1. MUST: HTTP requests in Emscripten environments MUST use JavaScript Fetch API or XMLHttpRequest as the transport layer, invoked via message queue boundaries (_fetcher.send, js_xhr.send)

## Policy Block

- MUST HTTP requests in Emscripten environments MUST use JavaScript Fetch API or XMLHttpRequest as the transport layer, invoked via message queue boundaries (_fetcher.send, js_xhr.send)
