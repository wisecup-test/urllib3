...
1. MUST: All HTTP message transmission at public API boundaries MUST use socket.send() or connection.send() as the terminal operation for byte sequence delivery

## Policy Block

- MUST All HTTP message transmission at public API boundaries MUST use socket.send() or connection.send() as the terminal operation for byte sequence delivery
