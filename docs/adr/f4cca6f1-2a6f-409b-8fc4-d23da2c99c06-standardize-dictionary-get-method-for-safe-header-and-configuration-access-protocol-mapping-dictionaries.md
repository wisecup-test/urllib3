...
1. MUST: Protocol mapping dictionaries (port_by_scheme, HASHFUNC_MAP, _SSL_VERSION_TO_TLS_VERSION) MUST be accessed via .get() with sensible fallback defaults

## Policy Block

- MUST Protocol mapping dictionaries (port_by_scheme, HASHFUNC_MAP, _SSL_VERSION_TO_TLS_VERSION) MUST be accessed via .get() with sensible fallback defaults
