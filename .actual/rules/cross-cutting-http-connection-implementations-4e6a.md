# Standardize Socket-Level Send Operations for HTTP Protocol Implementation: Http Connection Implementations
...
- **R-HTTP-CONN-001** MUST: HTTP/2 connection implementations MUST coordinate send operations with stream multiplexing to prevent frame interleaving corruption.
...
- HTTP/2 send operations are centralized in `http2/connection.py` with explicit threading coordination and stream state management

<enforcement>
Claude Code MUST NOT skip or defer verification. Integration test failures block merge until send implementation is corrected. Code review MUST identify missing send() methods or incorrect signatures before approval. Runtime errors from incorrect send usage trigger bug reports and regression test creation.
</enforcement>